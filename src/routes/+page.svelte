				...
				<div class="flex gap-10 flex-wrap">
					...
					<div class="">
						...
						<div class="flex gap-4">
							<button
								class={`btn btn-circle bg-purple-500 hover:bg-purple-600 border-none border-gray-300 ${
									stress === 'purple' && 'border-8'
								}`}
								on:click={() => {
									(stress = 'purple'), textarea.focus();
								}}
							/>
							<button
								class={`btn btn-circle bg-yellow-400 hover:bg-yellow-600 border-none border-gray-300 ${
									stress === 'yellow' && 'border-8'
								}`}
								on:click={() => {
									(stress = 'yellow'), textarea.focus();
								}}
							/>
							<button
								class={`btn btn-circle bg-green-400 hover:bg-green-600 border-none border-gray-300 ${
									stress === 'green' && 'border-8'
								}`}
								on:click={() => {
									(stress = 'green'), textarea.focus();
								}}
							/>
							<div class="flex-1" />
						</div>
					...
					<div>
						...
						<div class="flex gap-4 mt-4">
							...
							<button
								...
								on:click={() => {
									(tick = !tick), textarea.focus(), (touched = false);
								}}
							>
				...
				<div class="cursor-pointer label block">
					<label class="flex gap-2">
						<svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 256 256"
							><path
								fill="currentColor"
								d="M152 86a30 30 0 1 0-30-30a30 30 0 0 0 30 30Zm0-48a18 18 0 1 1-18 18a18 18 0 0 1 18-18Zm66.49 100.86c-.59.27-7.17 3.13-18.88 3.13c-13.86 0-34.9-4-61.73-21a165.89 165.89 0 0 1-17.43 36.51c9.43 2.78 22 7.72 33.19 16.26C172.46 188.05 182 207.65 182 232a6 6 0 0 1-12 0c0-44-37.23-59.18-56.91-64.11q-1.2 1.55-2.46 3.09c-19.25 23.31-43.34 35.45-70.11 35.45a90.72 90.72 0 0 1-9.12-.43a6 6 0 0 1 1.2-12c26.63 2.66 49.77-7.66 68.77-30.69c13.16-15.94 21.94-35.51 26.08-49.15c-40.51-24.52-66.59-4.78-67.72-3.89a6 6 0 0 1-7.48-9.38c.37-.3 9.39-7.43 24.76-10c13.86-2.31 35.92-1.3 62.36 16.67c47.14 32 73.88 20.47 74.14 20.35a6 6 0 1 1 5 10.92Z"
							/></svg
						>
					...
					<label class="flex gap-2">
						<svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"
							><g
								...
								strokeWidth="1.5"
								><path d="m12.44 9.127l-1.408 5.635l4.93 6.339m-5.634-2.817L8.215 21.1" /><path
									d="M8.215 13.353c0-3.944 2.817-4.226 4.226-4.226h1.408c.235 1.174 1.268 3.663 3.522 4.226M13 7a2 2 0 1 0 0-4a2 2 0 0 0 0 4Z"
								/></g
							></svg
						>
	...
	<ul>
		{#each modes as mode}
			<li class="flex gap-2 align-center text-md hover:bg-base-100 rounded">
				<div
					class={`-translate-y-4 circle ${mode.stress === 'purple' && 'bg-purple-500'} ${
						mode.stress === 'yellow' && 'bg-yellow-500'
					} ${mode.stress === 'green' && 'bg-green-500'} w-10 h-10 rounded-xl`}
				/>
				<div>
					<div class="flex gap-3 items-center">
						{#if mode.tick}
							<span
								class={`flex items-center justify-center rounded-full w-5 h-5 text-white bg-orange-500 border-none -rotate-45`}
							>
						...
						{#if mode.touched}
							<span
								class={`flex items-center justify-center rounded-full w-5 h-5 text-white bg-red-600 border-none`}
							>
								<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
									><path
										fill="currentColor"
